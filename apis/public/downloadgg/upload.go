...
const upload = "https://download.gg/server/upload.php"

func (b *dlg) DoUpload(name string, size int64, file io.Reader) error {
	...
	ref := "https://download.gg/file-" + strings.Replace(string(body), "&", "_", -1)
	b.resp = ref
	return nil
}
...
func (b dlg) newMultipartUpload(config uploadConfig) ([]byte, error) {
	if config.debug {
